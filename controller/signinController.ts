    ...
    if (idRegex.test(req.body.user_id) && pwRegex.test(req.body.user_pw)) {
      ...
      pool
        ...
        .then(async (value: any) => {
          //비밀번호 검증
          const hashedPw = await bcrypt.compare(
            req.body.user_pw,
            value[0][0].user_pw
          );
          if (hashedPw) {
            //access 토큰 생성
            let accessToken = await createToken("access", value[0][0].user_id);
            //refresh 토큰 생성
            ...
            console.log("hashed password fail");
            res.status(404).json({ msg: "비밀번호 불일치 입니다." });
          }
        })
        .catch((error: Error) => {
